...
func NewPluginManager(liveConfig *Config, u string) *PluginManager {
...
func (m *PluginManager) AutoLoad(config *Config) error {
	...
	pluginDir := config.Server.Plugin.Dir
	...
	for _, f := range files {
		...
		if err != nil {
			slog.Error(fmt.Sprintf("Erreur chargement plugin %s : %v", f.Name(), err))
			return err
		}
	}
...
func (m *PluginManager) loadPlugin(path string, config map[string]string) error {
	...
	// 3. On regarde si le plugin implémente aussi l'interface de commande SSH
	if raw, err := rpcClient.Dispense("commander"); err == nil && raw != nil {
		if c, ok := raw.(Commander); ok {
			newPlugin.commander = c
		}
	}
