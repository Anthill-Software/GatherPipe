...
func (g *ExporterRPCClient) Export(metrics []Metric) error {
	var resp struct{} // Obligatoire pour net/rpc même si on ne s'en sert pas
	return g.client.Call("Plugin.Export", metrics, &resp)
}
...
func (s *ExporterRPCServer) Export(metrics []Metric, resp *struct{}) error {
	return s.Impl.Export(metrics)
}
