...
func (m *PluginManager) renderInstall(term *term.Terminal, config *Config, args []string) {
	...
	if err := os.MkdirAll(config.Server.Plugin.Dir, 0755); err != nil {
		term.Write([]byte(fmt.Sprintf(Prefix.Error+" Impossible de créer le dossier %s : %v\r\n", config.Server.Plugin.Dir, err)))
		return
	}

	// 4. Préparer l'URL et le chemin
	binaryURL, destPath := resolveURL(target.BinaryURL, target.Version, config.Server.Plugin.Dir, target.Name)
